...
var j = class extends Material {
  constructor({
    ...
    o.uniforms = this.uniforms = n, o.vertexShader = this.vertexShader = _ || "", o.fragmentShader = this.fragmentShader = A || "", o.update({
  ...
  update({
    ...
    const S = P(_ || ""), d = P(a || ""), i = this;
    A && (i.uniforms = A), _ && (i.vertexShader = _), a && (i.fragmentShader = a), Object.entries(w).forEach(([s, n]) => {
      for (const f in n) {
        const t = n[f];
        (d && d.includes(t) || S && S.includes(t)) && (i[s] || (i[s] = 1));
      }
    });
    const R = i.__csm.prevOnBeforeCompile, o = (s, n, f) => {
      let t, l = "";
      if (n) {
        ...
        if (r !== -1) {
          l = n.slice(0, r);
          let c = 0, g = -1;
          for (let M = r; M < n.length; M++)
            if (n[M] === "{" && c++, n[M] === "}" && (c--, c === 0)) {
              g = M;
              break;
            }
          if (g !== -1) {
            const M = n.slice(r, g + 1);
            t = M.slice(M.indexOf("{") + 1, -1);
          }
        } else
          l = n;
      }
      if (f && (n && n.includes(e.fragColor)) && t && (t = `csm_UnlitFac = 1.0;
` + t), s.includes("//~CSM_DEFAULTS")) {
        s = s.replace(
          "void main() {",
          `
        ...
        const r = s.lastIndexOf("//~CSM_MAIN_END");
        if (r !== -1) {
          const c = `
            ${t ? `${t}` : ""}
            //~CSM_MAIN_END
          `;
          s = s.slice(0, r) + c + s.slice(r);
        }
      } else {
    ...
    i.onBeforeCompile = (s, n) => {
      R == null || R(s, n);
      const f = p || {}, t = i.type, l = t ? `#define IS_${t.toUpperCase()};
` : `#define IS_UNKNOWN;
`;
      s.vertexShader = l + `#define IS_VERTEX
` + s.vertexShader, s.fragmentShader = l + `#define IS_FRAGMENT
` + s.fragmentShader;
      const T = (r) => {
        for (const c in r) {
          const g = c === "*" || S && S.includes(c);
          if (c === "*" || d && d.includes(c) || g) {
            const E = b[c];
            if (E && E !== "*" && (Array.isArray(E) ? !E.includes(t) : E !== t)) {
              console.error(
            ...
            for (const I in $) {
              const h = $[I];
              if (typeof h == "object") {
                const N = h.type, L = h.value;
                N === "fs" ? s.fragmentShader = s.fragmentShader.replace(
                  I,
                  L
                ) : N === "vs" && (s.vertexShader = s.vertexShader.replace(
                  I,
                  L
                ));
              } else h && (s.vertexShader = s.vertexShader.replace(
                I,
                h
      ...
      T(k), T(f), s.vertexShader = o(
        ...
        d,
        true
      ), A && (s.uniforms = { ...s.uniforms, ...i.uniforms }), i.uniforms = s.uniforms;
    };
    const C = i.customProgramCacheKey;
    i.customProgramCacheKey = () => ((m == null ? void 0 : m()) || U((S || "") + (d || ""))) + (C == null ? void 0 : C.call(i)), i.needsUpdate = true;
  }
  clone() {
    const a = this;
