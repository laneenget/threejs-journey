...
var _tempBuffer = new Float32Array(6);
var BVH = class {
  constructor() {
    this._roots = null;
...
// node_modules/three-mesh-bvh/src/core/ObjectBVH.js
...
// node_modules/three-mesh-bvh/src/core/SkinnedMeshBVH.js
var _v0 = new Vector3();
var _v1 = new Vector3();
var _v2 = new Vector3();
var _ray5 = new Ray();
var _inverseMatrix5 = new Matrix4();
var _localPoint = new Vector3();
var IS_GT_REVISION_1692 = parseInt(REVISION) >= 169;
var IS_LT_REVISION_1612 = parseInt(REVISION) <= 161;
...
function getProjectedDistance(ray, vec2) {
  return _tempVec.subVectors(vec2, ray.origin).dot(ray.direction);
}
...
function computeDisjointEdges(geometry, unmatchedSet, eps) {
  ...
  for (let i = 0, l = edges.length; i < l; i++) {
    ...
    _v02.fromBufferAttribute(posAttr, i0);
    _v12.fromBufferAttribute(posAttr, i1);
    toNormalizedRay(_v02, _v12, _ray6);
    let info;
    let commonRay = rays.findClosestRay(_ray6);
    if (commonRay === null) {
      commonRay = _ray6.clone();
    ...
    let start = getProjectedDistance(commonRay, _v02);
    let end = getProjectedDistance(commonRay, _v12);
    if (start > end) {
      [start, end] = [end, start];
    }
    if (_ray6.direction.dot(commonRay.direction) < 0) {
...
function getHitSide(tri, bvh) {
  tri.getMidpoint(_ray7.origin);
  tri.getNormal(_ray7.direction);
  const hit = bvh.raycastFirst(_ray7, DoubleSide);
  const hitBackSide = Boolean(hit && _ray7.direction.dot(hit.face.normal) > 0);
...
function getHitSideWithCoplanarCheck(tri, bvh) {
  ...
  _ray7.direction.copy(_normal);
  tri.getMidpoint(_ray7.origin);
  const total = 3;
  let count = 0;
  let minDistance = Infinity;
  for (let i = 0; i < total; i++) {
    _ray7.direction.x += rand() * JITTER_EPSILON;
    _ray7.direction.y += rand() * JITTER_EPSILON;
    _ray7.direction.z += rand() * JITTER_EPSILON;
    _ray7.direction.multiplyScalar(-1);
    const hit = bvh.raycastFirst(_ray7, DoubleSide);
    let hitBackSide = Boolean(hit && _ray7.direction.dot(hit.face.normal) > 0);
    if (hitBackSide) {
      count++;
    }
    if (hit !== null) {
...
var PointsHelper = class extends InstancedMesh {
  ...
  setPoints(points) {
    for (let i = 0, l = points.length; i < l; i++) {
      ...
      _matrix5.makeTranslation(point.x, point.y, point.z);
      this.setMatrixAt(i, _matrix5);
    }
...
function getOverlapEdge(tri1, e1, tri2, e2, target) {
  ...
  toNormalizedRay(v0_1, v1_1, _ray8);
  let d0_1 = _vec6.subVectors(v0_1, _ray8.origin).dot(_ray8.direction);
  let d1_1 = _vec6.subVectors(v1_1, _ray8.origin).dot(_ray8.direction);
  if (d0_1 > d1_1) [d0_1, d1_1] = [d1_1, d0_1];
  let d0_2 = _vec6.subVectors(v0_2, _ray8.origin).dot(_ray8.direction);
  let d1_2 = _vec6.subVectors(v1_2, _ray8.origin).dot(_ray8.direction);
  if (d0_2 > d1_2) [d0_2, d1_2] = [d1_2, d0_2];
  const final_0 = Math.max(d0_1, d0_2);
  const final_1 = Math.min(d1_1, d1_2);
  _ray8.at(final_0, target.start);
  _ray8.at(final_1, target.end);
}
